...
def get_and_analitik_skills(params):
    ...
    found = result['found']
    print(f'По запросу {params["text"]} найдено количество вакансий:  {found}')
...
def get_analitik_skills(params):     # Парсер для бота
    domain='https://api.hh.ru/'
    url=f'{domain}vacancies'
    params['page'] = 0
    ...
    result = requests.get(url, params=params).json()
    found = result['found']
    str_skills = f'По запросу {params["text"]} найдено: {found} вакансий\n'
    print(str_skills)
    ...
    result = sorted(dict_skills.items(), key=lambda x:x[1], reverse=True)
    print(f'Всего требований:  {qiantity_skill}')
    str_skills += f'Всего требований:  {qiantity_skill} \n'
    skills_bot_number = 1
    for element in result:
        element_ = f'{element[0]} в {element[1]} вакансиях из {x}'
        list_skills.append(element_)
        str_skills += f'{element_} \n'
        skills_bot_number += 1
        if skills_bot_number == 51:
            break
        print(element_)

    print('')
    list_salary = []
    salary_1 = (f'Средн зп с выч налг:  {average_salary_gross_false} , по {(count_salary_min_gross_false + count_salary_max_gross_false) / 2} вакансиям из {x}')
    list_salary.append(salary_1)
    salary_2 = (f'Сред зп без выч налг:  {average_salary_gross_true}  , по {(count_salary_min_gross_true + count_salary_max_gross_true) / 2} вакансиям из {x}')
    list_salary.append(salary_2)
    salary_3 = (f'Сред мин зп с выч налг:  {average_min_salary_gross_false}  , по {count_salary_min_gross_false} вакансиям  из {x}')
    list_salary.append(salary_3)
    salary_4 = (f'Сред макс зп с выч налг:  {average_max_salary_gross_false}  , по {count_salary_max_gross_false} вакансиям  из {x}')
    list_salary.append(salary_4)
    salary_5 = (f'Сред мин зп без выч налг:  {average_min_salary_gross_true}  , по {count_salary_min_gross_true} вакансиям  из {x}')
    list_salary.append(salary_5)
    salary_6 = (f'Сред макс зп без выч налг:  {average_max_salary_gross_true}  , по {count_salary_max_gross_true} вакансиям  из {x}')
    list_salary.append(salary_6)
    str_salary = ''
    for element in list_salary:
        str_salary += f'{element}\n'
        print(element)
    return str_skills, str_salary
